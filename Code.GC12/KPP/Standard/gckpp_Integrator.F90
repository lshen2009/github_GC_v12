...
SUBROUTINE INTEGRATE_0( TIN, TOUT, LS_type,LS_NSEL, LS_NDEL,&
  Prate, Lrate, &
...
SUBROUTINE INTEGRATE_1( TIN, TOUT, LS_type,LS_NSEL, LS_NDEL,&
  ...
  ICNTRL_U, RCNTRL_U, ISTATUS_U, RSTATUS_U, IERR_U )
   ...
   END IF
        ...
        CALL Rosenbrock_1(LU_NSEL_1,VAR_selected,TIN,TOUT,ATOL,RTOL,&
          RCNTRL,ICNTRL,RSTATUS,ISTATUS,IERR, &
...
!~~~>   Compute the function at current time
   CALL FunTemplate_1(T,Y,Fcn0, LS_NSEL, LS_type)
   ISTATUS(Nfun) = ISTATUS(Nfun) + 1
...
!~~~>   Compute the Jacobian at current time
   CALL JacTemplate_1(T,Y,Jac0,LS_NSEL, LS_LU_NONZERO, LS_LU_IROW, LS_LU_ICOL, LS_type)   
   ISTATUS(Njac) = ISTATUS(Njac) + 1
...
Stage: DO istage = 1, ros_S
	 ...
	 Ynew(1:N) = Y(1:N)
         ...
         END DO
         Tau = T + ros_Alpha(istage)*Direction*H
         CALL FunTemplate_1(Tau,Ynew,Fcn, LS_NSEL, LS_type)
         ISTATUS(Nfun) = ISTATUS(Nfun) + 1
       END IF ! if istage == 1 elseif ros_NewF(istage)
       !slim: CALL WCOPY(N,Fcn,1,K(ioffset+1),1)
       K(ioffset+1:ioffset+N) = Fcn(1:N)
...
!~~~> Local variables
   ...
   Delta = SQRT(Roundoff)*MAX(DeltaMin,ABS(T))
   CALL FunTemplate_1(T+Delta,Y,dFdT, LS_NSEL, LS_type)
   ISTATUS(Nfun) = ISTATUS(Nfun) + 1
   CALL WAXPY(N,(-ONE),Fcn0,1,dFdT,1)
   CALL WSCAL(N,(ONE/Delta),dFdT,1)
...
SUBROUTINE FunTemplate_1( T, Y, Ydot, LS_NSEL, LS_type )
 ...
 INTEGER,INTENT(IN)::LS_NSEL, LS_type
   ...
   Told = TIME
   TIME = T
   CALL Fun_1( Y, FIX, RCONST, Ydot, LS_NSEL)
   TIME = Told
END SUBROUTINE FunTemplate_1

SUBROUTINE JacTemplate_1( T, Y, Jcb, LS_NSEL, LS_LU_NONZERO, LS_LU_IROW, LS_LU_ICOL, LS_type)
 USE gckpp_Global, ONLY: FIX, RCONST, TIME
 USE gckpp_Jacobian
 USE gckpp_LinearAlgebra
